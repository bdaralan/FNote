...
class ObjectGeneratorTests: XCTestCase {
    ...
    func testImportPublicCollection() {
        ...
        let importPublicCollection = expectation(description: "importPublicCollection")
        
        generator.importPublicCollection(publicCollection, with: [card01, card02, card03, card04]) { collection in
            
            // assert collection
            XCTAssertEqual(collection.name, "\(publicCollection.name) by \(publicCollection.authorName)")
            ...
            importPublicCollection.fulfill()
        }
        
        wait(for: [importPublicCollection], timeout: 3)
    }
}
